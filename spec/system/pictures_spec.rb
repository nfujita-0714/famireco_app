  ...
  describe '検索機能テスト' do
    ...
    context '投稿者検索の場合' do
      it '検索した投稿者の写真投稿が表示される' do
        fill_in 'q[user_name_cont]', with: 'user02'
        click_on '検索'
        expect(page).to have_content 'user02'
        expect(page).to have_no_content 'user01'
      end
    end
    context '投稿日で検索をした場合' do
      it '検索した投稿日の範囲の投稿のみが表示される' do
        fill_in 'q[created_at_gteq]', with: '002021-09-01'
        fill_in 'q[created_at_lteq]', with: '002021-09-30'
        click_on '検索'
        expect(page).to have_content '2021-09-10'
        expect(page).to have_no_content '2021-10-10'
      end
    end
  end
